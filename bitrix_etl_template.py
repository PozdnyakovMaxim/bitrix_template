...
def require_filled(name: str, value: str) -> str:
    if value.startswith("<") and value.endswith(">"):
        raise RuntimeError(f"Заполни {name} в верхнем блоке констант")
    return value


def build_mysql_url() -> str:
    user = quote_plus(require_filled("MYSQL_USER", MYSQL_USER))
    password = quote_plus(require_filled("MYSQL_PASSWORD", MYSQL_PASSWORD))
    return f"mysql+pymysql://{user}:{password}@{MYSQL_HOST}:{MYSQL_PORT}/{MYSQL_DB}?charset={MYSQL_CHARSET}"
...
def build_postgres_url() -> str:
    ...
    return f"postgresql+psycopg2://{user}:{password}@{POSTGRES_HOST}:{POSTGRES_PORT}/{POSTGRES_DB}"
...
SELECT
    ...
    buu.UF_1C_PR51E769A662EB AS main_organization_name
FROM b_utm_user uu
...
WHERE i.ID IS NULL
  AND e1.IBLOCK_ID = 76
  AND s.STATE_TITLE = 'Заявка выполнена'
...
TABLE_DDLS = (
    """
    CREATE TABLE IF NOT EXISTS bitrix.absense (
        user_id bigint,
    ...
    """,
    """
    CREATE TABLE IF NOT EXISTS bitrix.employees (
    ...
    """,
    """
    CREATE TABLE IF NOT EXISTS bitrix.local_trip (
    ...
    """,
    """
    CREATE TABLE IF NOT EXISTS bitrix.remote_work (
        bitrix_employee_id bigint,
...
def import_sqlalchemy():
    try:
        ...
        raise RuntimeError("Установи зависимости: sqlalchemy, pymysql, psycopg2-binary") from exc
...
def create_source_engine(cfg: BitrixConfig):
    try:
        ...
        raise RuntimeError("Установи pymysql") from exc
...
def create_target_engine(cfg: BitrixConfig):
    try:
        ...
        raise RuntimeError("Установи psycopg2-binary") from exc
    create_engine, _ = import_sqlalchemy()
...
def replace_table(conn, cfg: BitrixConfig, spec: QuerySpec, rows: List[Dict[str, Any]]) -> None:
    ...
    conn.execute(text(f"TRUNCATE TABLE {destination}"))
...
def upsert_table(conn, cfg: BitrixConfig, spec: QuerySpec, rows: List[Dict[str, Any]]) -> None:
    ...
    if non_key_columns:
        assignments = ", ".join(f"{ident(column)} = s.{ident(column)}" for column in non_key_columns)
        changed = " OR ".join(f"t.{ident(column)} IS DISTINCT FROM s.{ident(column)}" for column in non_key_columns)
        update_sql = text(f"""
...
def write_rows(target_engine, cfg: BitrixConfig, rows_by_table: Dict[str, List[Dict[str, Any]]]) -> None:
    ...
    with target_engine.begin() as conn:
        ...
        if cfg.run_scud_staff_report_procedure:
            log.info(f"Calling {SCUD_STAFF_REPORT_PROCEDURE}")
            conn.execute(import_sqlalchemy()[1](f"CALL {SCUD_STAFF_REPORT_PROCEDURE}()"))
        if cfg.run_reports_presense_procedure:
            log.info(f"Calling {REPORTS_PRESENSE_PROCEDURE}")
            conn.execute(import_sqlalchemy()[1](f"CALL {REPORTS_PRESENSE_PROCEDURE}()"))
